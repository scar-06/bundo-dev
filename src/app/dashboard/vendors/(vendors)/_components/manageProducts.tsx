...
import { DeleteCircleIcon, EditCircleIcon, GreenLocationIcon } from "@/assets";
...
function EditProductCard({ pictures, name, _id }: Partial<Product>) {
...
function ManageProducts() {
  ...
  return (
    <div>
      <div className="mb-8">
        <GoBack text="Manage Products" noMargin />
      </div>
        ...
        {products && products?.length > 0 ? (
          <>
            {products.map((product) => (
              <EditProductCard {...product} key={product.name} />
            ))}
          </>
        ) : (
          <div className="my-20 flex w-full items-center justify-center">
            <VendorsEmptyState
              ...
              descriptionOne=" No product to display now"
              descriptionTwo={`You can upload via storefront`}
            />
          </div>
        )}
      </div>
