    ...
    <div className="w-full ">
      <div className="flex w-full justify-between">
        ...
        <div className="flex items-center gap-1">
          <GreenChatIcon />
          <button onClick={() => action()} className="text-[12px] lg:hidden">
            Leave a review
          </button>
          <span className="text-[12px] mmlg:hidden">Leave a review</span>
        </div>
      </div>
      ...
      {reviews?.map((review) => (
        <div className="flex w-full flex-col" key={Math.random() * Math.PI}>
          <div className="my-6 flex w-full justify-between">
            <div className="flex w-1/2 flex-grow items-center gap-2 text-wrap">
              <Button className="bg-[#DEF2FB] text-lg font-bold text-[#5BC0EB]">
                {review?.fullName.slice(0, 1)}
              </Button>
              <p className="max-w-[60%] whitespace-normal text-wrap  text-[12px]">
                {review?.review}
              </p>
            </div>
            <div className="my-auto flex w-max bg-white ">
