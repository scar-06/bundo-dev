...
import { getAllVendorProducts } from "@/lib/app/vendors/services";
import { useGetBusiness } from "@/hooks/useGetBusiness";
import PageLoader from "@/components/loaders/pageLoader";
import ServiceView from "@/components/shared/cards/serviceView";

function ViewVendorServices({ productId }: { productId: string }) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { data: businessData, isFetching: fetchingBusiness } = useGetBusiness();
  ...
  if (isFetching || fetchingBusiness) {
    return (
      <div>
        <PageLoader />
      </div>
  ...
  if (isError) {
    return <div>Error fetching a vendor product</div>;
  }
  const products = sortByMatchingKey(data?.products ?? [], "_id", productId);
  return (
...
export default ViewVendorServices;
