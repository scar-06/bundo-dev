...
function GoBack({ text }: Props) {
  const router = useRouter();
  return (
    <div>
      <button
        onClick={router.back}
        className=" flex h-full w-full cursor-pointer items-center gap-1 text-sm font-semibold md:text-base lg:text-lg "
      >
        <svg
          width="24"
          height="24"
        ...
        <span className="mx-auto text-sm font-medium md:text-base">{text}</span>{" "}
      </button>
    </div>
  );
}
