...
function VendorProducts() {
  ...
  const sortedProducts = productId
    ...
    : products;
  const getAvailableSortedProducts = sortedProducts?.filter(
    (product) => product.quantity > 0,
  );
  return (
    <div>
      {productId && (
        <div className=" mb-4 block w-full bg-white  lg:py-4 ">
          <button
            onClick={router.back}
      ...
      {productId ? (
        getAvailableSortedProducts?.map((product) => (
        ...
        <div className=" flex min-h-[calc(100vh_-_200px)]">
          <VendorsEmptyState descriptionOne="Click on a product item to see more details" />
        </div>
      )}
    </div>
  );
